...
function handleTryClick(event){
    ...
    if(Number(inputNumber.value) == randomNumber){
        ...
        document.querySelector(".screen2 h2").innerText = ` Parabéns você acertou, o número escolhido é:  ${randomNumber}` // mostrar o resultado
        randomNumber = Math.round(Math.random()*10) // numeros aleatorios e inteiros
    }else{
        acertos++
        document.getElementById("tentenovamente").innerText = `tente novamente, você errou` // mostrar o resultado
    }
    inputNumber.value = ""
}
